...
[<Test>]
let ``One file, no match, case sensitive`` () =
    let pattern = "forbidden"
    let flags = ""
    let files = [paradiseLostFileName]
...
[<Test>]
let ``One file, one match, match entire lines flag without hits`` () =
    let pattern = "With loss of Eden"
    let flags = "-x"
    let files = [paradiseLostFileName]
...
[<Test>]
let ``One file, one match, multiple flags, no hit`` () =
    let pattern = "KIng of MEN."
    let files = [iliadFileName]
    let flags = "-n -i -x"
    let expected = ""
