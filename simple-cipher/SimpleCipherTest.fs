...
[<Test>]
let ``Encode wraps around`` () =
    Assert.That(encode "b" "z", Is.EqualTo("a"))

[<Test>]
let ``Decode wraps around`` () =
    Assert.That(decode "b" "a", Is.EqualTo("z"))
...
let ``Encode random uses key made of letters`` () =
    ...
    Assert.That(System.Text.RegularExpressions.Regex.IsMatch(key, "[a-z]+"))
