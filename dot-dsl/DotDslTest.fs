...
[<Test>]
let ``Graph with one edge with attributes`` () =    
    let g = graph [
                edge "a" "b" [("color", "blue")]
            ]             

    Assert.That(nodes g, Is.Empty)
    Assert.That(edges g, Is.EqualTo([edge "a" "b" [("color", "blue")]]))
    Assert.That(edges g, Is.Not.EqualTo([edge "a" "b" [("color", "green")]]))
    Assert.That(attrs g, Is.Empty)
...
let ``Graph with one attribute`` () = 
    ...
    Assert.That(attrs g, Is.Not.EqualTo([attr "bar" "2"]))
