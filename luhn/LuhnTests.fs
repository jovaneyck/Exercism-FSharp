...
let ``Addends doubles every other number from the right`` () =
    Assert.That(addends 2121L, Is.EqualTo [4; 1; 4; 1])

[<Test>]
let ``Addends other doubles every other number from the right`` () =
...
[<Test>]
let ``Addends subtracts 9 when doubled number is a multiple of 9`` () =
    Assert.That(addends 91L, Is.EqualTo [9; 1])
